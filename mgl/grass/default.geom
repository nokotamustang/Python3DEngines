...
float grass_size;
float dist_length;
float lod1_dist = 1.0f;
float lod2_dist = 1.0f;
float lod3_dist = 1.0f;
...
const mat4 model45 = mat4(cos(rot_45), 0, sin(rot_45), 0,
			 	0, 1.0, 0, 0,
				-sin(rot_45),	0, cos(rot_45), 0,
				0, 0, 0,	1);
const mat4 modelm45 = mat4(cos(-rot_45), 0, sin(-rot_45), 0,
			 	0, 1.0, 0, 0,
				-sin(-rot_45),	0, cos(-rot_45), 0,
				0, 0, 0,	1);
const float rot_90 = radians(90);
const mat4 model90 = mat4(cos(rot_90), 0, sin(rot_90), 0,
			 	0, 1.0, 0, 0,
				-sin(rot_90),	0, cos(rot_90), 0,
				0, 0, 0,	1);
...
void main() {
    ...
    grass_size = random(in_pos.xz) * grass_scale * (1.0f - c_min_size) + c_min_size;
	float t = 6.0f;
    if (dist_length > LOD2) {
        t *= 1.5f;
    }
	dist_length += (random(in_pos.xz)*t - t/2.0f);
	if (dist_length > LOD4) {
		return;
	}
	int detail_level = 4;
